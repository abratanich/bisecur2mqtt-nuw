...
class MCPPacket:
    ...
    def to_bytes(self):
        ...
        return (self.buffer + MCPPacket.calc_checksum(self.buffer).to_bytes(1, byteorder='big',
                                                                            signed=False)).hex().upper().encode()
...
class MCPSetState:
    ...
    @staticmethod
    def construct(port_id):
        return MCPSetState(port_id)

    def to_bytes(self):
        data = bytes([self.port_id, self.state])
        return data

    def __repr__(self):
        return f"MCPSetState(port_id={self.port_id}, state=0x{self.state:02X})"
...
class MCPSetValue:
    ...
    def to_bytes(self):
        return self.value_id.to_bytes(1, byteorder='big', signed=False) + self.value.to_bytes(1, byteorder='big',
                                                                                              signed=False)
...
class MCPLogin:
    ...
    def to_bytes(self):
        return len(self.username).to_bytes(1, byteorder='big',
                                           signed=False) + self.username.encode() + self.password.encode()
...
MCP2Command = {
    MCPPing: 0,
    MCPGetMAC: 2,
    MCPSetValue: 3,       # требуется авторизация
    MCPGetValue: 4,       # требуется авторизация
    MCPDebug: 5,
    JCMP: 6,
    MCPGETGWVersion: 7,   # без авторизации
    MCPLogin: 16,
    MCPLogout: 17,
    MCPGetUserIds: 32,    # требуется авторизация
    MCPGetUserName: 33,   # требуется авторизация
    MCPAddUser: 34,       # без авторизации
    MCPChangePassword: 35,
    MCPRemoveUser: 36,
    MCPSetUserRights: 37, # требуется авторизация
    MCPGetName: 38,       # без авторизации
    MCPSetName: 39,       # требуется авторизация
    MCPGetUserRights: 40, # требуется авторизация
    MCPChangePasswordOfUser: 69,  # команда не найдена в некоторых случаях
    MCPScanWifi: 81,
    MCPWifiFound: 82,
    MCPGetWifiState: 83,
    MCPGetTransition: 112,
    MCPSetState: 51,      # для управления состоянием (например, для входных ворот через MCPSetState)
