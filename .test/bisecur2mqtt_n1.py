...
def do_door_action(action, set_door):
    ...
    if action in "impulse up down partial light stop":
        try:
            port = int(set_door)
            # if port == "0":
            #     # Для входных ворот используем MCPSetState, который формирует payload "00FF"
            mcp_cmd = MCPSetState.construct(port)
                # value = "00FF"  # Пример для логирования
            # elif port == "1":
            #     value = int("01FF92A5", 16)
            #     mcp_cmd = MCPGenericCommand.construct(CMD_SET_STATE, value)
            # else:
            #     raise Exception("Unknown door id")
            print(mcp_cmd)
            action_resp = CLI.generic(mcp_cmd, False)
            # Если для входных ворот (id 0) устройство не возвращает ответ, можно его обработать отдельно
            if action_resp is None and port == "0":
                log.info("No response for door 0; assuming command executed (but physical state unchanged)")
                # Здесь можно, например, оповестить через MQTT, что ответ не получен,
                # но физически ворота не изменили состояние – это сигнал к дальнейшей проверке.
                return None
            elif action_resp is None:
                raise Exception("No response received from device")
            # Отправка команды
            # action_resp = CLI.generic(cmd, False)
            # port = 3 # 1open garage | 2opengarage 1/2 | 3 open main 1/2
            # value = port << 8 | 0xFF
            # mcp_cmd = MCPGenericCommand.construct(CMD_SET_STATE, 1000)
            # action_resp = CLI.generic(mcp_cmd, False)
            #print(mcp_cmd.to_bytes().hex())
            #resp = CLI.get_transition(set_door)
            #print(resp)
            #data_to_send = mcp_cmd.to_bytes()  # Преобразование в байты
            #print(data_to_send)
            #action_resp = CLI.generic(mcp_cmd, False)
